...
func main() {
	...
	// Pre-allocate scanners (1 per worker) to avoid memory leaks
	scanners := make([]*scanner.WeChatQRScanner, *workers)
	for i := 0; i < *workers; i++ {
		sc, err := scanner.NewWeChatQRScanner(*modelsDir)
		if err != nil {
			log.Fatalf("Falha ao inicializar scanner para o worker %d: %v", i, err)
		}
		scanners[i] = sc
	}
	fmt.Printf("Initialized %d WeChatQRScanner instances.\n", *workers)
	...
	for length := 1; length <= *maxLength; length++ {
		...
		currentLevelPipelines := fuzz.GeneratePipelinesOfLength(localFactories, length)
		fmt.Printf("Phase %d has %d pipeline permutations.\n", length, len(currentLevelPipelines))

		// We test one pipeline at a time across all unsolved images.
		// This ensures true BFS: simplest filters on ALL images first.
		for _, pipeTemplate := range currentLevelPipelines {
			names := pipeTemplate.Names()
			fmt.Printf("\n--- Testing Pipeline: %s ---\n", names)
			...
			if len(imagesToProcess) == 0 {
				fmt.Printf("All images solved! Skipping remaining pipelines in phase.\n")
				break
			}
			...
			fmt.Printf("Processing %d unsolved images with pipeline %s (%d workers)...\n", len(imagesToProcess), names, *workers)
			...
			for w := 0; w < *workers; w++ {
				...
				go func(workerID int) {
					...
					localSc := scanners[workerID]

					// Each worker recreates the exact pipeline for this template to ensure thread-safety
					namesList := strings.Split(names, "|")
					var workerFilters []fuzz.Filter
					for _, n := range namesList {
						workerFilters = append(workerFilters, localFactories[n]())
					}
					workerPipe := fuzz.Pipeline{Filters: workerFilters}
					...
					for f := range imageChan {
						...
						for workerPipe.Next() {
							start := time.Now()
							dst, ok := workerPipe.Apply(img)
							var results []string
							if ok && !dst.Empty() {
								results = localSc.DetectRaw(dst)
							}
							dst.Close() // Close() em Mat vazio é no-op seguro no gocv

							if len(results) > 0 {
								dur := time.Since(start).Milliseconds()
								configs := workerPipe.Configs()
								
								// Evita data-race e garante que só 1 worker anote o sucesso
								if _, alreadySolved := solvedImages.LoadOrStore(f.Name(), true); alreadySolved {
									break
								}
